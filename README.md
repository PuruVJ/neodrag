...
- 🤏 Tiny - Only [1.4KB](https://bundlephobia.com/package/svelte-drag@latest) min+gzip
...
| cancel               | `cancel`                         | `undefined`                 | CSS Selector of an element inside the parent node(on which `use:draggable` is applied). If it is provided, Trying to drag inside the `cancel` selector will prevent dragging.                                                                                                                                  |
