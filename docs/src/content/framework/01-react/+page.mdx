...
Import built-in plugins from `@neodrag/core/plugins` or `@neodrag/react/plugins`. Drop targets and sortable lists use `@neodrag/react/drop` (`useDroppable`, `sortable`, `useSortableItem`). Same reactive `plugins` / `() => plugins` shape as `useDraggable`.
