...
Import built-in plugins from `@neodrag/core/plugins` or `@neodrag/react/plugins`. Use `useDroppable(ref, plugins)` for drop targets (same reactive `plugins` / `() => plugins` shape as `useDraggable`).
