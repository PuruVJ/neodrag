...
`@neodrag/core` powers the framework packages. Prefer those entry points in apps; use core directly for custom integrations or plugins.
...
<script>
  NeoDrag.Neodrag.shared.draggable(document.getElementById('drag'), [
    NeoDrag.axis('x'),
    NeoDrag.grid([10, 10]),
  ]);
</script>
