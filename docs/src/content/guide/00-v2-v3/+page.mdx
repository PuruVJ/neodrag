...
<script>
  import { Draggable } from '@neodrag/svelte'
	import { axis, bounds, BoundsFrom, grid, events } from '@neodrag/svelte/plugins';
...
<script>
  ...
  let isConstrained = $state(false);
	...
	const drag_0 = new Draggable({ plugins: [() => [axisComp, boundsComp].filter(Boolean)] });
</script>
...
```tsx
import { useDraggable, defineDragPlugin } from '@neodrag/solid';
