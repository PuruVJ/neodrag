...
// New imports
import { createDraggable } from '@neodrag/solid';
import { axis, bounds, BoundsFrom } from '@neodrag/solid/plugins';
```
...
import { createDraggable } from '@neodrag/solid';
import { axis, bounds, BoundsFrom, grid } from '@neodrag/solid/plugins';
...
| `threshold: { distance: 5 }` | `new Draggable({ threshold: { distance: 5 }, plugins: [] })` | Per-binding option (not a plugin) |
| `threshold: { delay: 100 }`  | `new Draggable({ threshold: { delay: 100 }, plugins: [] })`  | Time delay threshold              |
...
- **v2:** `offsetX`, `offsetY`, `rootNode`, `currentNode`, `event` (`PointerEvent`)
- **v3:** `offset`, `offsetPx`, `rootNode`, `visualNode`, `input` (`InteractionInput`), `pointer` — use `data.input.native` when `data.input.kind === 'pointer'`
...
<script>
  import { Draggable } from '@neodrag/svelte';
  import { axis, bounds, BoundsFrom } from '@neodrag/svelte/plugins';

  let currentAxis = $state('x');
  let isConstrained = $state(false);

  const drag_0 = new Draggable({
    plugins: [() => [axisComp, boundsComp].filter(Boolean)],
  });
</script>
...
function App() {
  ...
  createDraggable(element, [axisComp, boundsComp]);
...
<script>
  import { Draggable } from '@neodrag/svelte';
  import { defineDragPlugin } from '@neodrag/svelte/plugins';

  const loggerPlugin = defineDragPlugin(() => ({
    key: Symbol('logger'),
    setup(ctx) {
  ...
  const drag_0 = new Draggable({ plugins: [loggerPlugin()] });
</script>

<div {@attach drag_0.attachment}>Custom behavior</div>
```
...
```tsx
import { useDraggable, defineDragPlugin } from '@neodrag/react';
...
```vue
<script setup>
import { vDraggable, defineDragPlugin } from '@neodrag/vue';
...
```tsx
import { createDraggable, defineDragPlugin } from '@neodrag/solid';
...
function CustomExample() {
  const [element, setElement] = createSignal();
  createDraggable(element, [loggerPlugin]);
...
```typescript
import { Neodrag, defineDragPlugin } from '@neodrag/vanilla';
...
const instance = Neodrag.shared.draggable(element, [loggerPlugin]);
```
...
<script>
  import { Draggable } from '@neodrag/svelte';
  import { scrollLock } from '@neodrag/svelte/plugins';
...
<div {@attach drag_0.attachment}>No page scroll while dragging</div>
```
...
```typescript
import { Neodrag, scrollLock } from '@neodrag/vanilla';
...
- Actions become attachments: `use:draggable` → `{@attach drag.attachment}` with `new Draggable({ plugins })`
- Reactive declarations: `let var = value` → `let var = $state(value)`
- Event handlers: `on:click` → `onclick`
- Use `Compartment.of` for reactive plugins
...
- `createDraggable()` pattern → `createDraggable()` hook
- `use:draggable` directive → `createDraggable(element, plugins)`
...
- Function call → Class instantiation: `draggable(el, opts)` → `Neodrag.shared.draggable(el, plugins)`
