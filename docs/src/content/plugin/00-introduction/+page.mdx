...
<div {@attach drag_0.attachment}>Drag me around!</div>
```
...
import { grid, axis, bounds, BoundsFrom } from '@neodrag/solid/plugins';
...
- `onDrag` / `onDragStart` / `onDragEnd` on `new Draggable({ ... })` — drag lifecycle callbacks (not a plugin)

## Reactive plugin lists

For plugins that change based on user interaction, pass a reactive factory:
...
<script>
  ...
  const drag_0 = new Draggable({ plugins: [() => [axisComp]] });
</script>
...
function DynamicAxis() {
  ...
  createDraggable(element, [axisComp]);
...
```typescript
import { Neodrag, axis } from '@neodrag/vanilla';

let currentAxis = 'x';
const instance = Neodrag.shared.draggable(element, () => [axisComp]);
...
> **Important:** v3 wrappers call `engine.update` when your `() => plugins` factory re-runs. Simply changing plugin options or recreating arrays won't update the draggable.
...
const logger = defineDragPlugin(() => ({
  key: Symbol('my:logger'),
...
const logger = defineDragPlugin((prefix = 'DRAG') => ({
  key: Symbol('my:logger'),
...
interface DragPlugin {
  key: symbol; // Unique identity — use Symbol('my-plugin') for a readable description
  phase?: 'pre' | 'resolve' | 'post';
  init?: (ctx) => state;
  start?: (ctx, state, event) => boolean | void;
  drag?: (ctx, state, event) => { x?: number; y?: number } | void;
  end?: (ctx, state, event, reason) => void;
  destroy?: (ctx, state) => void;
}
```
...
export const awesomePlugin = defineDragPlugin(
  (options: AwesomePluginOptions = {}) => ({
    key: Symbol('awesome:plugin'),
...
const map = Neodrag.shared.instances;
console.log('Active draggables:', map.size);

for (const [element, instance] of map) {
...
With `@neodrag/core` directly, use `Neodrag.shared.instances` on your engine instance.
