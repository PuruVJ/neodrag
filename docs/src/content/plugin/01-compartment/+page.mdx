...
<script>
  ...
  const axisComp = Compartment.of(() => axis(confirmedAxis));
...
<script>
  ...
  const debugComp = Compartment.of(() =>
...
<script>
  ...
  let gridSize = $state(20);
  const axisComp = Compartment.of(() => axis(userAxisChoice));
  const gridComp = Compartment.of(() => grid([gridSize, gridSize]));

  // ❌ Bad use - static values
  const staticBounds = Compartment.of(() =>
    bounds(BoundsFrom.parent()),
  );
