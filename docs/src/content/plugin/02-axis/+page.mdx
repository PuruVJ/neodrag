...
Change axis while dragging using compartments:
...
```typescript
// Horizontal movement with 20px grid snapping
const plugins = [axis('x'), grid([20, 20])];

// Vertical slider constrained to parent
const plugins = [axis('y'), bounds(BoundsFrom.parent())];

// Plugin order matters - apply axis constraint first
const plugins = [
