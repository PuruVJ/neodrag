...
<script>
  ...
  const drag_0 = new Draggable({ plugins: [axis('x')] });
  const drag_1 = new Draggable({ plugins: [axis('y')] });
  const drag_2 = new Draggable({ plugins: [axis(null)] });
</script>

<!-- Horizontal slider -->
<div {@attach drag_0.attachment}>Horizontal only</div>

<!-- Vertical scrollbar -->
<div {@attach drag_1.attachment}>Vertical only</div>

<!-- Free movement -->
<div {@attach drag_2.attachment}>Any direction</div>
```
...
import { axis, createDraggable } from '@neodrag/solid';
...
```typescript
import { axis, Neodrag } from '@neodrag/vanilla';

const horizontal = document.getElementById('horizontal');
const vertical = document.getElementById('vertical');
const free = document.getElementById('free');

Neodrag.shared.draggable(horizontal, [axis('x')]);
Neodrag.shared.draggable(vertical, [axis('y')]);
Neodrag.shared.draggable(free, [axis(null)]);
```
...
Neodrag.shared.draggable(element, () => [axisComp]);
...
Neodrag.shared.draggable(handle, [
  axis('x'),
  bounds(BoundsFrom.element(track)),
  events({ onDrag: handleDrag }),
]);
