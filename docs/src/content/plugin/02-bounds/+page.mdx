...
<div {@attach drag_0.attachment}>Can't leave the screen</div>

<!-- Stay within parent -->
<div class="container">
  <div {@attach drag_1.attachment}>Contained child</div>
</div>
```
...
import { bounds, BoundsFrom, createDraggable } from '@neodrag/solid';
...
function BoundedElements() {
  ...
  createDraggable(viewport, [bounds(BoundsFrom.viewport())]);
  createDraggable(parent, [bounds(BoundsFrom.parent())]);
...
```typescript
import { bounds, BoundsFrom, Neodrag } from '@neodrag/vanilla';

const viewportElement = document.getElementById('viewport-bound');
const parentElement = document.getElementById('parent-bound');

Neodrag.shared.draggable(viewportElement, [
  bounds(BoundsFrom.viewport()),
]);
Neodrag.shared.draggable(parentElement, [
...
<div {@attach drag_0.attachment}>Stay in window</div>

<!-- Viewport with padding -->
<div {@attach drag_1.attachment}>20px padding from edges</div>
```
...
function ViewportBounds() {
  ...
  createDraggable(basic, [bounds(BoundsFrom.viewport())]);
  createDraggable(padded, [
    bounds(
      BoundsFrom.viewport({
        top: 20,
        left: 20,
...
Neodrag.shared.draggable(basic, [bounds(BoundsFrom.viewport())]);
Neodrag.shared.draggable(padded, [
  bounds(
    BoundsFrom.viewport({
      top: 20,
      left: 20,
...
<script>
  ...
  const drag_0 = new Draggable({
    plugins: [bounds(() => BoundsFrom.element(container))],
  });
  const drag_1 = new Draggable({
    plugins: [bounds(BoundsFrom.selector('#drop-zone'))],
  });
</script>

<div bind:this={container} class="boundary-container">
  <div {@attach drag_0.attachment}>Bound to container</div>
</div>

<!-- Using selector -->
<div class="zone" id="drop-zone">
  <div {@attach drag_1.attachment}>Bound by selector</div>
</div>
```
...
function ElementBounds() {
  ...
  createDraggable(bounded, [
    bounds(() => BoundsFrom.element(container()!)),
  ]);
  createDraggable(selector, [bounds(BoundsFrom.selector('#drop-zone'))]);
...
Neodrag.shared.draggable(bounded, [
  bounds(() => BoundsFrom.element(container)),
]);
Neodrag.shared.draggable(selector, [
  bounds(BoundsFrom.selector('#drop-zone')),
]);
```
...
<script>
  import { Draggable } from '@neodrag/svelte';
  import { bounds } from '@neodrag/svelte/plugins';
  ...
  const drag_0 = new Draggable({ plugins: [bounds(circularBounds)] });
  const drag_1 = new Draggable({ plugins: [bounds(timeBounds)] });
</script>

<div {@attach drag_0.attachment}>Circular boundary</div>

<div {@attach drag_1.attachment}>Animated boundary</div>
```
...
```tsx
import { createSignal } from 'solid-js';
import { bounds, createDraggable } from '@neodrag/solid';
...
function CustomBounds() {
  ...
  createDraggable(circular, [bounds(circularBounds)]);
  createDraggable(time, [bounds(timeBounds)]);
...
```typescript
import { bounds, Neodrag } from '@neodrag/vanilla';
...
Neodrag.shared.draggable(circular, [bounds(circularBounds)]);
Neodrag.shared.draggable(time, [bounds(timeBounds)]);
```
...
bounds(boundaryFunction, (ctx) => {
  ...
  return ctx.hook === 'start' || ctx.hook === 'end';
});
```
...
- On setup (if `shouldRecompute` returns true for `'setup'`)
- On drag start (if `shouldRecompute` returns true for `'start'`)
- During drag (if `shouldRecompute` returns true for `'drag'`)
- On drag end (if `shouldRecompute` returns true for `'end'`)
