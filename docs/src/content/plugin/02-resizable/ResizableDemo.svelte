<script lang="ts">
	...
	// Every edge + corner handle from one iteration over RESIZE_EDGES, each bound type-safely via
	// resize.handle(edge). Bounded to the stage; reads back the live size.
	const resize = new Resizable({ minWidth: 90, minHeight: 80, bounds: 'parent' });
</script>

<DocDemo label="Resize from any edge or corner" hint="8 handles from RESIZE_EDGES + resize.handle(edge)">
	{#snippet stage()}
