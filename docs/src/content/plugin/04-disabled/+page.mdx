...
```svelte
<script>
  import { draggable, disabled } from '@neodrag/svelte';
</script>
...
```svelte
<script>
  import { draggable, disabled, Compartment } from '@neodrag/svelte';
...
```typescript
import { disabled, Neodrag, Compartment } from '@neodrag/vanilla';
