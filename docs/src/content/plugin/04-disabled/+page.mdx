...
<script>
  ...
  const disabledComp = Compartment.of(() => disabled(isEditMode));
</script>

<div {@attach draggable(() => [disabledComp])} class="card">
...
<template>
  <div v-draggable="() => [disabledComp]" class="card">
    <div class="card-header">
      <h3>Card Title</h3>
...
### Permission-Based Dragging

```typescript
// Disable based on user permissions
const permissionComp = new Compartment(() => disabled(!canUserDrag));

// Disable for readonly mode
const readonlyComp = new Compartment(() => disabled(isReadonly));
```
...
The disabled plugin is simple but effective:
