...
<script>
  ...
  const velocityTracker = events({
    onDrag: (data) => {
      const now = Date.now();
      const deltaTime = now - lastTime;

      if (deltaTime > 0) {
        velocity.x = Math.round(
          ((data.offset.x - lastPosition.x) / deltaTime) * 1000,
        );
        velocity.y = Math.round(
...
<div {@attach draggable([velocityTracker])}>
...
const plugins = [
  events({
    onDrag: (data) => {
      ...
      if (deltaTime > 0) {
        velocity.value.x = Math.round(
          ((data.offset.x - lastPosition.x) / deltaTime) * 1000,
        );
        velocity.value.y = Math.round(
          ((data.offset.y - lastPosition.y) / deltaTime) * 1000,
        );
      }
