...
<script>
  ...
  import { grid } from '@neodrag/svelte/plugins';

  const drag_0 = new Draggable({ plugins: [grid([20, 20])] });
  const drag_1 = new Draggable({ plugins: [grid([50, 30])] });
  const drag_2 = new Draggable({ plugins: [grid([25, null])] });
  const drag_3 = new Draggable({ plugins: [grid([null, 20])] });
</script>

<!-- Square 20px grid -->
<div {@attach drag_0.attachment}>Snaps to 20px squares</div>

<!-- Rectangular grid -->
<div {@attach drag_1.attachment}>Snaps to 50x30px rectangles</div>

<!-- Horizontal-only snapping -->
<div {@attach drag_2.attachment}>Only snaps horizontally</div>

<!-- Vertical-only snapping -->
<div {@attach drag_3.attachment}>Only snaps vertically</div>
```
...
import { grid, createDraggable } from '@neodrag/solid';
...
function GridExamples() {
  ...
  createDraggable(square, [grid([20, 20])]);
  createDraggable(rect, [grid([50, 30])]);
  createDraggable(horizontal, [grid([25, null])]);
  createDraggable(vertical, [grid([null, 20])]);
...
```typescript
import { grid, Neodrag } from '@neodrag/vanilla';
...
Neodrag.shared.draggable(square, [grid([20, 20])]);
Neodrag.shared.draggable(rect, [grid([50, 30])]);
Neodrag.shared.draggable(horizontal, [grid([25, null])]);
Neodrag.shared.draggable(vertical, [grid([null, 20])]);
```
...
<script>
  ...
  const drag_0 = new Draggable({ plugins: [() => [gridComp]] });
</script>
...
function DynamicGrid() {
  ...
  createDraggable(element, [gridComp]);
...
Neodrag.shared.draggable(element, () => [gridComp]);
...
<script>
  ...
  const drag_0 = new Draggable({ plugins: [grid([100, 20])] });
  const drag_1 = new Draggable({ plugins: [grid([60, null])] });
  const drag_2 = new Draggable({ plugins: [grid([80, 120])] });
</script>

<!-- Wide columns, short rows -->
<div {@attach drag_0.attachment}>100x20px grid</div>

<!-- Timeline-style: hourly columns -->
<div {@attach drag_1.attachment}>60px intervals (hourly)</div>

<!-- Calendar-style: daily columns, weekly rows -->
<div {@attach drag_2.attachment}>Daily x Weekly</div>
```
...
function AsymmetricGrids() {
  ...
  createDraggable(wide, [grid([100, 20])]);
  createDraggable(timeline, [grid([60, null])]);
  createDraggable(calendar, [grid([80, 120])]);
...
Neodrag.shared.draggable(wide, [grid([100, 20])]);
Neodrag.shared.draggable(timeline, [grid([60, null])]);
Neodrag.shared.draggable(calendar, [grid([80, 120])]);
```
...
<script>
  ...
  const responsiveGrid = grid([columnWidth, 40]),
  );
  ...
  function handleResize() {
	...
	const drag_0 = new Draggable({ plugins: [() => [responsiveGrid]] });
</script>
...
const responsiveGrid = grid([columnWidth.value, 40]),
);
...
import { grid } from '@neodrag/solid/plugins';
...
function ResponsiveGrid() {
  ...
  createDraggable(element, [responsiveGrid]);
...
const responsiveGrid = grid([getColumnWidth(), 40]),
);

const element = document.getElementById('element');
const statusEl = document.getElementById('status');

Neodrag.shared.draggable(element, () => [responsiveGrid]);
...
1. **Calculate grid position** - Uses `Math.round(value / gridSize) * gridSize` to find the next grid point
