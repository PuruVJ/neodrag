...
Update grid settings using compartments:
...
<input type="range" min="10" max="50" bind:value={gridSize} />
...
<template>
  <div>
    ...
    <input type="range" min="10" max="50" v-model="gridSize" />
...
```typescript
// Grid snapping within bounded area
...
// Plugin order matters - apply grid before bounds
const plugins = [
...
function LayoutEditor() {
  ...
  useDraggable(widgetRef, [
    isSnapEnabled ? grid([snapSize, snapSize]) : grid([null, null]),
...
function LayoutEditor() {
  ...
  return (
    <div>
      <div ref={setCanvas} class="editor-canvas">
        <div ref={setWidget} class="widget">
          Widget{' '}
          {isSnapEnabled() ? `(${snapSize()}px grid)` : '(free move)'}
        </div>
      </div>
...
let instance = new Draggable(widget, [
  getGridPlugin(),
  bounds(BoundsFrom.element(canvas)),
]);
