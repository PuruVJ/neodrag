...
<script>
  ...
  let defaultPos = $state({ x: 0, y: 0 });
	...
	const drag_0 = new Draggable({ plugins: [() => [position({ current: currentPos })]] });
	const drag_1 = new Draggable({ plugins: [() => [position({ default: defaultPos })]] });
</script>
...
function PositionExample() {
  ...
  useDraggable(initialRef, () => [position({ default: defaultPos })]);
...
<template>
  <div v-draggable="() => [position({ current: currentPos })]">Controlled position</div>
...
// Controlled position
const controlled = Neodrag.shared.draggable(element, () => [position({ current: currentPos })]);
...
<template>
  <div v-draggable="() => [position({ current: elementPosition }), events({ onDrag: (data) => { elementPosition = data.offset; } })]">
    Position synced with state
  </div>
...
function SyncedPosition() {
  ...
  // Note: Solid uses array format, not function format
  useDraggable(divRef, () => [position({ current: elementPosition() }), events({ onDrag: (data) => setElementPosition(data.offset) })]);
