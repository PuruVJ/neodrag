...
function PositionExample() {
  ...
  const { as: controlledDiv } = createDraggable(() => [
    position({ current: currentPos() }),
  ]);

  const { as: initialDiv } = createDraggable(() => [
    position({ default: { x: 0, y: 0 } }),
  ]);

  return (
    <div>
      <controlledDiv>Controlled position</controlledDiv>
      <initialDiv>Initial position, then user controls</initialDiv>
      <button onClick={() => setCurrentPos({ x: 200, y: 100 })}>
        Move to (200, 100)
      </button>
    </div>
...
```tsx
import { createSignal } from 'solid-js';
import { position, events, createDraggable } from '@neodrag/solid';
...
function SyncedPosition() {
  ...
  return (
    <div>
      <div>Position synced with state</div>
      <p>
        Current position: {elementPosition().x}, {elementPosition().y}
      </p>
    </div>
