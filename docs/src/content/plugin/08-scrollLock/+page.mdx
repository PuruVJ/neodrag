...
<script>
  ...
  const drag_0 = new Draggable({ plugins: [scrollLock()] });
  const drag_1 = new Draggable({
    plugins: [scrollLock({ lockAxis: 'x' })],
  });
  const drag_2 = new Draggable({
    plugins: [scrollLock({ container: () => scrollContainer })],
  });
</script>

<!-- Prevent all scrolling while dragging -->
<div {@attach drag_0.attachment}>Drag me - page won't scroll</div>

<!-- Only prevent horizontal scrolling -->
<div {@attach drag_1.attachment}>Vertical scrolling still works</div>

<!-- Target specific container -->
<div bind:this={scrollContainer} class="scroll-area">
  <div {@attach drag_2.attachment}>Lock specific container</div>
</div>
```
...
function ScrollLockExamples() {
  ...
  createDraggable(fullLock, [scrollLock()]);
  createDraggable(horizontal, [scrollLock({ lockAxis: 'x' })]);
...
```typescript
import { scrollLock, Neodrag } from '@neodrag/vanilla';
...
Neodrag.shared.draggable(fullLock, [scrollLock()]);
Neodrag.shared.draggable(horizontal, [scrollLock({ lockAxis: 'x' })]);
Neodrag.shared.draggable(container, [
  scrollLock({ container: () => scrollArea }),
]);
```
...
<script>
  ...
  let targetContainer = $state(null);
	...
	const drag_0 = new Draggable({ plugins: [() => [scrollLockComp]] });
</script>
...
import { scrollLock } from '@neodrag/solid/plugins';
...
function DynamicScrollLock() {
  ...
  createDraggable(element, [scrollLockComp]);
...
Neodrag.shared.draggable(element, () => [scrollLockComp]);
