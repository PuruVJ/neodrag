...
<script>
  import { Draggable } from '@neodrag/svelte'
	import { threshold } from '@neodrag/svelte/plugins';

	const drag_0 = new Draggable({ plugins: [threshold({ distance: 10 })] });
	const drag_1 = new Draggable({ plugins: [threshold({ delay: 300 })] });
	const drag_2 = new Draggable({ plugins: [threshold({ distance: 8, delay: 150 })] });
	const drag_3 = new Draggable({ plugins: [threshold(null)] });
</script>
...
<!-- Disable threshold -->
<div {@attach drag_3.attachment}>Immediate dragging</div>
```
...
```typescript
import { Neodrag, threshold } from '@neodrag/vanilla';
...
Neodrag.shared.draggable(distance, [threshold({ distance: 10 })]);
Neodrag.shared.draggable(delay, [threshold({ delay: 300 })]);
Neodrag.shared.draggable(both, [threshold({ distance: 8, delay: 150 })]);
Neodrag.shared.draggable(disabled, [threshold(null)]);
```
...
<script>
  ...
  let sensitivity = $state('normal');
	...
	const drag_0 = new Draggable({ plugins: [() => [thresholdComp]] });
</script>
...
Neodrag.shared.draggable(element, () => [thresholdComp]);
