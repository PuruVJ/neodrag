...
import { threshold, createDraggable } from '@neodrag/solid';

function ThresholdExamples() {
  const { as: distanceDiv } = createDraggable(() => [
    threshold({ distance: 10 }),
  ]);
  const { as: delayDiv } = createDraggable(() => [
    threshold({ delay: 300 }),
  ]);
  const { as: bothDiv } = createDraggable(() => [
    threshold({ distance: 8, delay: 150 }),
  ]);
  const { as: disabledDiv } = createDraggable(() => [
    threshold(null),
  ]);

  return (
    <div>
      <distanceDiv>Drag after moving 10px</distanceDiv>
      <delayDiv>Hold for 300ms to drag</delayDiv>
      <bothDiv>Move 8px AND wait 150ms</bothDiv>
      <disabledDiv>Immediate dragging</disabledDiv>
    </div>
  );
}
```
...
draggable(distance, [threshold({ distance: 10 })]);
draggable(delay, [threshold({ delay: 300 })]);
draggable(both, [threshold({ distance: 8, delay: 150 })]);
draggable(disabled, [threshold(null)]);
```
...
function DynamicThreshold() {
  ...
  const { as: draggableDiv } = createDraggable(() => [thresholdComp]);
...
draggable(element, [thresholdComp]);
