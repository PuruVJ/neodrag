...
```svelte
<script>
  import { draggable, threshold } from '@neodrag/svelte';
</script>
...
```svelte
<script>
  import { draggable, threshold, Compartment } from '@neodrag/svelte';
...
```javascript
import { Neodrag, threshold, Compartment } from '@neodrag/vanilla';
