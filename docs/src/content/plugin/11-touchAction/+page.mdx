...
<script>
  ...
  import { touchAction } from '@neodrag/svelte/plugins';

  const drag_0 = new Draggable({
    plugins: [touchAction('manipulation')],
  });
  const drag_1 = new Draggable({ plugins: [touchAction('none')] });
  const drag_2 = new Draggable({ plugins: [touchAction('pan-y')] });
  const drag_3 = new Draggable({ plugins: [touchAction(false)] });
</script>

<!-- Default manipulation mode -->
<div {@attach drag_0.attachment}>Touch-optimized dragging</div>

<!-- Disable all browser touch behaviors -->
<div {@attach drag_1.attachment}>Pure drag only</div>

<!-- Allow vertical scrolling while preventing horizontal -->
<div {@attach drag_2.attachment}>Vertical scroll + drag</div>

<!-- Don't set touch-action property -->
<div {@attach drag_3.attachment}>Browser default behavior</div>
```
...
```vue
<script setup>
import { touchAction } from '@neodrag/vue';
</script>
...
function TouchActionExamples() {
  const [manipulationRef, setManipulationRef] = createSignal();
  const [noneRef, setNoneRef] = createSignal();
  const [panYRef, setPanYRef] = createSignal();
  const [defaultRef, setDefaultRef] = createSignal();
  ...
  return (
    <div>
      <div ref={setManipulationRef}>Touch-optimized dragging</div>
      <div ref={setNoneRef}>Pure drag only</div>
      <div ref={setPanYRef}>Vertical scroll + drag</div>
      <div ref={setDefaultRef}>Browser default behavior</div>
    </div>
  );
}
```
...
```typescript
import { Neodrag, touchAction } from '@neodrag/vanilla';
...
Neodrag.shared.draggable(manipulation, [touchAction('manipulation')]);
Neodrag.shared.draggable(none, [touchAction('none')]);
Neodrag.shared.draggable(panY, [touchAction('pan-y')]);
Neodrag.shared.draggable(defaultElement, [touchAction(false)]);
```
...
<script>
  ...
  let allowScroll = $state(false);
	...
	const drag_0 = new Draggable({ plugins: [() => [touchActionComp]] });
</script>
...
import { touchAction } from '@neodrag/solid/plugins';
...
Neodrag.shared.draggable(element, () => [touchActionComp]);
