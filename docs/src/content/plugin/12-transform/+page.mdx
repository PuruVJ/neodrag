...
<div {@attach drag_0.attachment}>Standard dragging</div>
...
```vue
<script setup>
import { transform } from '@neodrag/vue';
...
```typescript
import { Neodrag, transform } from '@neodrag/vanilla';

const standard = document.getElementById('standard');
const custom = document.getElementById('custom');

Neodrag.shared.draggable(standard, [transform()]);

Neodrag.shared.draggable(custom, [
  transform((data) => {
    data.rootNode.style.transform = `
      translate(${data.offsetX}px, ${data.offsetY}px) 
      rotate(${data.offsetX * 0.5}deg)
...
<script>
  ...
  const skewTransform = (data) => {
	...
	const drag_0 = new Draggable({ plugins: [transform(scaleTransform)] });
	const drag_1 = new Draggable({ plugins: [transform(rotateTransform)] });
	const drag_2 = new Draggable({ plugins: [transform(skewTransform)] });
</script>
...
Neodrag.shared.draggable(scale, [transform(scaleTransform)]);
Neodrag.shared.draggable(rotate, [transform(rotateTransform)]);
Neodrag.shared.draggable(skew, [transform(skewTransform)]);
```
