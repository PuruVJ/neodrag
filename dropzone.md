...
interface DropPlugin<State = any> {
  ...
  dragEnter?(ctx: DropContext, state: State, dragData: unknown): boolean | void;
  dragOver?(ctx: DropContext, state: State, dragData: unknown): boolean | void;
  dragLeave?(ctx: DropContext, state: State): void;
  drop?(ctx: DropContext, state: State, dragData: unknown): boolean | void;
...
export function useDroppable(ref: RefObject<HTMLElement>, plugins: DropPlugin[]): DropState;
...
<script>
  ...
  function moveCard(cardData, targetColumn) {
    ...
    const sourceColumn = columns.find(c => c.cards.some(card => card.id === cardData.id));
...
[sortable({ items: () => todos, onReorder: setTodos }), insertionLine()]
...
export const analytics = unstable_definePlugin((config: {
  trackingId: string;
  events?: string[];
}) => ({
  ...
  setup(ctx) {
    console.log('Analytics plugin setup for dropzone', ctx.dropZone);
    return {
...
export const serverValidation = unstable_definePlugin((config: {
  endpoint: string;
  cacheTime?: number;
}) => ({
...
const myPlugin = unstable_definePlugin(() => ({
  ...
  drop(ctx, state, event) {
    console.log(`Dropping ${state.lastDataType} after ${state.enterCount} enters`);
  },
...
<template>
  <div 
    v-droppable="[validation({ accept: ['text/plain'] }), events({ onDrop })]"
...
[sortable({ items, onReorder }), accepts(['todo']), events({ onDrop: handler })]
