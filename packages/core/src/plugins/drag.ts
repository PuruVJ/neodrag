...
import { isPointerInput } from '../interaction-input.ts';
import { defineDragPlugin, type DragCtx, type DragPlugin } from '../types.ts';
import {
...
export const applyUserSelectHack: DragPlugin<UserSelectState> = {
	...
	start(ctx, state) {
		state.body_user_select_val =
			get_node_style(document.body, 'user-select') ?? document.body.style.webkitUserSelect ?? '';
...
function recomputeBounds(value: BoundFromFunction, ctx: import('../types.ts').DragCtx) {
...
export const autoScroll = defineDragPlugin(
	(
		...
		init(ctx) {
			const marginPx =
				options?.margin != null
					? resolveSizeInput(ctx.length, options.margin, sizeContext(ctx.rootNode, 'height'), 48)
					: 48;
			return {
				margin: marginPx,
...
export const ghost = defineDragPlugin(
	(options: { opacity?: number; zIndex?: number } = {}) => ({
	key: GHOST_KEY,
	phase: 'pre',
	...
	start(ctx, state) {
		...
		if (ctx.rootNode instanceof HTMLElement) {
			mirrorPresentationTree(ctx.rootNode, htmlClone);
		}
		htmlClone.removeAttribute('data-neodrag-state');
		htmlClone.removeAttribute('data-neodrag-count');
		htmlClone.dataset.ghostOriginLeft = String(rect.left);
		htmlClone.dataset.ghostOriginTop = String(rect.top);
		htmlClone.style.opacity = String(options.opacity ?? 0.5);
		...
		clone.style.setProperty('z-index', String(options.zIndex ?? GHOST_Z_INDEX), 'important');
