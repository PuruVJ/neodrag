...
export function keyboardMoveSensor(): Sensor & { key: symbol } {
	return {
		key: KEYBOARD_MOVE_SENSOR_KEY,
		...
		setup(host) {
			...
			const onKeyDown = (e: KeyboardEvent) => {
				...
				if (!root) {
					if (
						grab &&
						(e.key === grab.config.grabKey ||
							(grab.config.grabKey === 'Space' && e.key === ' '))
					) {
						e.preventDefault();
						releaseGrab();
					}
				...
				const isGrabKey = e.key === config.grabKey || (config.grabKey === 'Space' && e.key === ' ');
				if (isGrabKey) {
					e.preventDefault();
					if (grab?.node === root) {
						releaseGrab();
